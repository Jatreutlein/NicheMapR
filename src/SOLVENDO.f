...
C     USING endo_devel.R
      ...
      DOUBLE PRECISION FASKY,FASKYREF,FATDEN,FATPCT,FATTHK,FAVEG
      DOUBLE PRECISION FAVEGREF,FGDREF,FLSHVL,FLTYPE,FLYHR,FSKREF
      DOUBLE PRECISION FURTHRMK,FURTST,FURVARS,FURWET,GEND,GENV,GEOMout
      DOUBLE PRECISION GEOMVARS,GEVAP,GMASS,GR,GRAV,HC,HCFOR,HD,HCFREE
      DOUBLE PRECISION HDFORC,HDFREE,HTOVPR,INPUT,IPT,IRPROPout,KEFF
      DOUBLE PRECISION KFURCMPRS,KHAIR,KSUB,LEN,LHAIRD,LHAIRV,MASBAL
      ...
      DOUBLE PRECISION PCOND,PCTBAREVAP,PCTCO2,PCTDIF,PCTEYES
      DOUBLE PRECISION PCTN2,PCTO2,PCTWET,BLCMP,PCTWET_INC,PCTWET_LAST
      DOUBLE PRECISION PCTWET_MAX,PI,PR,PVEN,Q10,Q10mult
     ...
     & SOLARout(7),SIMULSOLout(2,16),SIMULOUT(16),FURVARS(13),
     & GEOMVARS(22),TRAITS(9),ENVVARS(18),ZBRENTin(17),ZBRENTout(15),
...
C      CORRECT FASKY FOR % VEGETATION SHADE OVERHEAD, ASHADE
     ...
     &  VEL,ZFUR,BP,ELEV,CONV_ENHANCE,GRAV,O2GAS,N2GAS,CO2GAS,CONVout)
     ...
     &  BLCMP,KFURCMPRS,CONV_ENHANCE,PVEN/)
        ENVVARS = (/FLTYPE,TA,TS,TBUSH,TVEG,TLOWER,TSKY,TCONDSB,RH,
     &   VEL,BP,ELEV,FASKY,FABUSH,FAVEG,FAGRD,QSLR,GRAV/)
     ...
     &   PCTEYES/)
        ...
        !# call SIMULSOL
        CALL SIMULSOL(DIFTOL,IPT,O2GAS,N2GAS,CO2GAS,FURVARS,GEOMVARS
     &   ,ENVVARS,TRAITS,TFA,PCTWET,TS,SIMULOUT)
...
C    &  VEL,ZFUR,BP,ELEV,CONV_ENHANCE,GRAV,O2GAS,N2GAS,CO2GAS,CONVout)
C
...
111   CONTINUE
      ...
      KFURV=SIMULSOLout(2, 16) ! successful solution found? (0 no, 1 yes)
     ...
     &  SHAPEB,SHAPEC,DHARA,RHOARA,PCOND,SAMODE,ORIENT,Z,GEOMout)
      ...
      ELSE
          ...
          TRADV = TSKCALCAVV
      ENDIF
      QIROUTD=2.*FASKY*sigma*EMISAN*ARADD*(TRADD+273.15)**4.
      QIRIND=QRADD*(-1.)+QIROUTD
      QIROUTV=2.*FAGRD*sigma*EMISAN*ARADV*(TRADV+273.15)**4.
     ...
     & KFURD, KFURV, KFURCMPRS, ZFURD, ZFURV, Q10/)
