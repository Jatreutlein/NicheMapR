...
C     FLTYPE = FLUID TYPE: 0 = AIR; 1 = FRESH WATER; 2 = SALT WATER
      ...
      SUBROUTINE SIMULSOL(DIFTOL,IPT,O2GAS,N2GAS,CO2GAS,FURVARS,GEOMVARS
     & ,ENVVARS,TRAITS,TFA,SKINW,TSKIN,RESULTS)
      ...
      DOUBLE PRECISION FURTHRMK,FURTST,FURVARS,FURWET,GEOMVARS,GRAV,HC
      DOUBLE PRECISION HD,HDFREE,IPT,IRPROPout,KEFF,KFUR,KRAD,LEN,NTRY
      DOUBLE PRECISION PCTBAREVAP,PCTEYES,PI,PVEN,QCOND,QCONV,QENV
      DOUBLE PRECISION QFSEVAP,QGENNET,QR1,QR2,QR3,QR4,QRAD,QRBSH,QRGRD
      DOUBLE PRECISION QRSKY,QRVEG,QSEVAP,QSLR,RESULTS,RFLESH,RFUR,RH
      DOUBLE PRECISION RRAD,RSKIN,SEVAPRES,SHAPE,SIG,SKINW,SOLCT,SOLPRO
      DOUBLE PRECISION SSQG,SUBQFAT,SUCCESS,SURFAR,TA,TBUSH,TC,TCONDSB
      DOUBLE PRECISION TFA,TFACALC,TFADIFF,TFAT1,TFAT2,TFAT3,TFAT4
      DOUBLE PRECISION TLOWER,TR,TRAITS,TRAPPX,TSKCALC,TSKCALC1
      DOUBLE PRECISION TSKCALC2,TSKCALCAV,TSKDIFF,TSKIN,TSKT1,TSKT2
      DOUBLE PRECISION TSKT3,TSKY,TVEG,VEL,VOL,XR,ZFUR,ZL
      DOUBLE PRECISION LHAIR,DHAIR,RHO,REFL,KHAIR,O2GAS,N2GAS,CO2GAS
      DOUBLE PRECISION BLCMP,CD1,CD2,CD3,CF1,KFURCMPRS,PCOND,RFURCMP
      DOUBLE PRECISION TFACMP,TFACMPT1,TFACMPT2,TR2,CONV_ENHANCE
      
      INTEGER I,S
     
      DIMENSION CONVRES(15),SEVAPRES(7),RESULTS(16)
      DIMENSION FURVARS(13) ! LEN,ZFUR,FURTHRMK,KEFF,BETARA,FURTST, ! NOTE KEFF DEPENDS ON IF SOLAR, AND THEN WHAT SIDE, SET IN R
      DIMENSION GEOMVARS(22) ! SHAPE,SUBQFAT,SURFAR,VOL,D,CONVAR,CONVSK,RFUR,RFLESH,RSKIN,XR,RRAD,ASEMAJ,BSEMIN,CSEMIN,CD,PVEN
      DIMENSION ENVVARS(18) ! FLTYPE,TA,TS,TBUSH,TVEG,TLOWER,TSKY,TCONDSB,RH,VEL,BP,ALT,FASKY,FABUSH,FAVEG,FAGRD,QSLR
      DIMENSION TRAITS(9) !TC,AK1,AK2,EMIS,FATTHK,FLYHR,FURWET,PCTBAREVAP,PCTEYES
      DIMENSION IRPROPout(26)

C     CONSTANTS      
      SIG = 5.6703744191844294E-8 ! W M-2 K-4
      PI = ACOS(-1.0D0)
...
5     CONTINUE
      ...
      DO 105, I=1,20
11     CONTINUE
       CALL CONV_ENDO(TSKIN,TA,SHAPE,SURFAR,FLTYPE,FURTST,D,TFA,VEL,
     &  ZFUR,BP,ALT,CONV_ENHANCE,GRAV,O2GAS,N2GAS,CO2GAS,CONVRES)
       HC=CONVRES(2)
       HD=CONVRES(5)
       HDFREE=CONVRES(6)
     ...
     & TFA, CONVAR, SEVAPRES)
       QSEVAP = SEVAPRES(1)
       QFSEVAP = SEVAPRES(7) 
       CALL IRPROP((0.7*TFA+0.3*TSKIN),DHAIR,DHAIR,LHAIR,LHAIR,ZFUR,
     & ZFUR,RHO,RHO,REFL,REFL,ZFUR,PVEN,KHAIR,IRPROPout)
       KEFF=IRPROPout(S+1)
       KFURCMPRS=IRPROPout(26)
...
C       NEED A TRAD APPROXIMATION FOR CALCULATING KRAD.
        ...
        KRAD = (16.0*SIG*(TRAPPX+273.15)**3.)/(3.*BETARA) ! EQ7 IN CONLEY AND PORTER 1986
        KFUR = KEFF+KRAD
       ENDIF
...
120   CONTINUE
      ...
      DO 140, I=1,20
125     CONTINUE
       CALL CONV_ENDO(TSKIN,TA,SHAPE,CONVSK,FLTYPE,FURTST,D,TFA,VEL,ZFUR
     &  ,BP,ALT,CONV_ENHANCE,GRAV,O2GAS,N2GAS,CO2GAS,CONVRES)
       HC=CONVRES(2)
       HD=CONVRES(5)
       HDFREE=CONVRES(6)
       CALL SEVAP_ENDO(BP,TA,RH,VEL,TC,TSKIN,ALT,SKINW,
